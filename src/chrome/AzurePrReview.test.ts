...
it("automatically opens a unique branch match but lets the user choose among multiple PRs", async () => {
  ...
  expect(document.body.textContent).toContain("Load threads");
  await cleanup();
  ...
  try {
    ...
    expect(document.body.textContent).not.toContain("Load threads");
    await act(async () =>
      [...document.querySelectorAll("button")]
        .find((button) =>
