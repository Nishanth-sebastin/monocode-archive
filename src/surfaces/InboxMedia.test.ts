...
it.each(["jira", "azure"])("loads authenticated %s images separately from agent context and retries failures", async (provider) => {
  ...
  const site = provider === "azure" ? "https://dev.azure.com/team" : "https://team.atlassian.net";
  const item = { provider, site, id: "42", url: provider === "azure" ? `${site}/Product/_workitems/edit/42` : `${site}/browse/ENG-42` } as InboxItem;
  ...
  try {
    ...
    expect(invoke).toHaveBeenCalledExactlyOnceWith(`${provider}_image`, { site: item.site, id: "42", attachmentId: "12" });
    ...
    expect(vi.mocked(invoke).mock.calls.every(([cmd]) => cmd === `${provider}_image`)).toBe(true);
