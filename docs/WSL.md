...
The installer is an unsigned test build of MonoCode Fork, with its existing app identity and profile. It updates an existing MonoCode Fork installation rather than installing an isolated preview. Windows may show an unknown-publisher warning. These artifacts do not publish a release or enable automatic updates. A packaged build does not establish live WSL acceptance. While Actions is blocked at the account level, no installer can be generated by this workflow.
